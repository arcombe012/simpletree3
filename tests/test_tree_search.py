...
class TestTreeSearch:
    def test_find_first(self):
        ...
        assert node_.key == 5  # type: ignore
        ...
        assert node_ is None

    def test_find_list(self):
        tree_ = build_simple_tree()
        nodes_ = list(find_nodes(tree_, key=5))
        assert [node_.key for node_ in nodes_] == [5]
    ...
    def test_find_list_rule(self):
        ...
        assert sorted(node_.key for node_ in nodes_) == list(range(5, _max_key))  # type: ignore
    ...
    def test_find_first_hinted(self):
        ...
        assert node_.key == 9  # type: ignore
        node_ = find_first_node_from_here(node_, key=0)
        assert node_.key == 0  # type: ignore
