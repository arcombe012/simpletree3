...
class TestTreeIteration:
    def test_preorder_iterator(self):
        ...
        assert po_ == _preorder_list, "preorder iterator result did not match"
    ...
    def test_filtered_preorder_iterator(self):
        root_ = build_simple_tree()
        po_ = list(n.key for n in \
            filtered_preorder_iterator(root_, select=lambda x: x.key % 2))  # type: ignore
        assert po_ == [k for k in _preorder_list if k % 2], \
        ...
        assert po_ == _preorder_list[:-4], "filtered preorder iterator result did not match (ignore)"
        ...
        po_ = list(n.key for n in \
            filtered_preorder_iterator(root_, ignore=lambda x: x.key == 2))
        assert po_ == _preorder_list[:-4], \
    ...
    def test_postorder_iterator(self):
        ...
        assert po_ == _postorder_list, "postorder iterator result did not match"
    ...
    def test_filtered_postorder_iterator(self):
        root_ = build_simple_tree()
        po_ = list(n.key for n in \
            filtered_postorder_iterator(root_, select=lambda x: x.key % 2))  # type: ignore
        assert po_ == [k for k in _postorder_list if k % 2], \
            "filtered postorder iterator result did not match (select)"

        po_ = list(n.key for n in \
            filtered_postorder_iterator(root_, ignore=lambda x: x.key == 2))
        assert po_ == _postorder_list[:-5] + [0], \
    ...
    def test_level_order_iterator(self):
        ...
        assert po_ == _level_order_list, "simple level order iterator result did not match"

        root_ = build_flex_tree()
        po_ = list(n.key for n in level_order_iterator(root_))
        assert po_ == _level_order_list, "flex level order iterator result did not match"

    def test_filtered_level_order_iterator(self):
        root_ = build_simple_tree()
        po_ = list(n.key for n in \
            filtered_level_order_iterator(root_, select=lambda x: x.key % 2))  # type: ignore
        assert po_ == [k for k in _level_order_list if k % 2], \
            "simple filtered level order iterator result did not match (select)"

        po_ = list(n.key for n in \
            filtered_level_order_iterator(root_, ignore=lambda x: x.key == 2))
        ...
        assert len(po_) == 0, "simple filtered level order iterator result did not match (ignore all)"
        ...
        po_ = list(n.key for n in \
            filtered_level_order_iterator(root_, ignore=lambda x: x.key == 0))
        assert len(po_) == 0, "flex filtered level order iterator result did not match (ignore all)"

    def test_leaves_iterator(self):
        root_ = build_simple_tree()
        lil_ = list(n.key for n in leaves_iterator(root_))
        leaves_ = [9, 10, 11, 12, 13, 14, 15, 6, 7, 8]
        assert lil_ == leaves_, "leaves iterator result did not match"
    ...
    def test_filtered_leaves_iterator(self):
        ...
        lil_ = list(n.key for n in \
            filtered_leaves_iterator(root_, _select, _ignore))
